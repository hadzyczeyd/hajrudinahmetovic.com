<?php 
	...
	if($action == 'add')
	{
		...
		if($_SERVER['REQUEST_METHOD'] == "POST")
		{
			...
			if(empty($errors))
			{
				...
				$query = "insert into predavanja (title,image,file,user_id,category_id,date,views,slug) values (:title,:image,:file,:user_id,:category_id,:date,:views,:slug)";
	...
	if($action == 'edit')
	{
		...
		$query = "select * from predavanja where id = :id limit 1";
		...
		if($_SERVER['REQUEST_METHOD'] == "POST" && $row)
		{
			...
			if(empty($errors))
			{
				...
				$query = "update predavanja set title = :title,user_id =:user_id,category_id =:category_id";
	...
	if($action == 'delete')
	{
		...
		if($_SERVER['REQUEST_METHOD'] == "POST" && $row)
		{
			...
			if(empty($errors))
			{
				...
				$query = "delete from predavanja where id = :id limit 1";
				db_query($query,$values);
  		...
  		<?php else:?>
  			...
  			<?php
				...
				$offset = ($page - 1) * $limit;
  				...
  				$query = "select * from predavanja order by id desc limit $limit offset $offset";
  				$rows = db_query($query);
